...
void traverse_root(uint8_t *image_buf, struct bpb33* bpb) {
    ...
    for (i = 0; i < bpb->bpbRootDirEnts; i++) {
        ...
        if (dirent->deAttributes == ATTR_NORMAL) { // deal with normal files
...
int foster_single_orphan(int orphan_count, uint16_t curr_cluster, uint8_t *image_buf, struct bpb33* bpb) {
    ...
    int clusters_size = calculate_size(curr_cluster, image_buf, bpb);
    append_clusters(cluster);
...
int count_clusters(struct direntry *dirent, 
    ...
    uint16_t cluster_size = bpb->bpbBytesPerSec * bpb->bpbSecPerClust;
    int byte_count = 0; uint16_t prev_cluster = cluster;
    append_clusters(cluster);
    if (is_end_of_file(cluster)) byte_count = 512;
...
void chkerr(struct direntry* dirent, char* filename, 
    ...
    if (num_clusters != 0 && entry_size < num_clusters - 512 ) { // believe the dir entry; fix the FAT
        printf("OUT OF BOUNDS: %s (size in directory entry: %d, size in FAT chain: %d) \n", filename, entry_size, num_clusters);
